...
RabbitMQ introduces the concept of **Exchanges**, which act as routing engines. Producers don't send messages directly to queues; they send them to an Exchange, which uses rules (bindings and routing keys) to determine which queue(s) should receive the message.
