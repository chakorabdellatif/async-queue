...
When a broker sends a message to a consumer, it doesn't instantly delete it. Instead, it marks the message as "unacknowledged" or "in-flight". If the consumer finishes its job, it sends an ACK, and the broker safely deletes the message. If the consumer crashes or times out before sending the ACK, the broker automatically re-queues the message so another worker can try.
