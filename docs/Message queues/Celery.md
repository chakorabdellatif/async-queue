...
## Concept Explanation
Celery is not a message broker itself; it is a **Distributed Task Queue framework** written in Python. It provides the abstractions to easily write tasks, send them, and run workers that consume them. It requires an external message broker (like RabbitMQ or Redis) to actually transport the messages.

Celery handles the complexity of polling the broker, managing worker processes (using pre-forking, gevent, or eventlet), handling retries, and writing results to a backend.
